        ...
        <div class="p-3 container-p-y">
            <div class="card">
                <div class="d-flex align-item-center p-4 justify-content-between w-100">
                    <h5 class="card-header p-0">{{ __('home.food') }}</h5>
                    <div class="d-flex align-item-center gap-3">
                        <button id="deleteSelected" class="btn btn-danger">
                            <i class="fa-regular fa-trash-can"></i>
                        </button>
                        <a href="{{ route('coach.food.create') }}">
                        ...
                        <tbody>
                            @foreach ($data as $food)
                                <tr>
                                    <th scope="row">
                                        <div class="d-flex align-items-center gap-2">
                                            <input class="form-check-input row__check" type="checkbox" value="{{ $food->id }}" />
                                            {{ $food->name }}
                                        </div>
                                    </th>
                                    <td class="w-px-50 h-auto rounded">
                                        <div class="d-flex align-items-center gap-2">
                                             {{ $food ? image_url($food->img) : asset('assett/img/hero/hero-2.jpg') }}
                                        </div>
                                    </td>
                                    <td> {{ $food->amount }} {{ __('home.g') }} </td>
                                    <td> {{ $food->fat }} {{ __('home.g') }}</td>
                                    <td> {{ $food->protein }} {{ __('home.g') }}</td>
                                    <td> {{ $food->carbohydrate }} {{ __('home.g') }}</td>
                                    ...
                                    <td class="">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item" href="{{ route('coach.food.edit', $food->id) }}"><i
                                                        class="bx bx-edit-alt me-1"></i> {{ __('home.Edit') }}</a>
                                                <a class="dropdown-item cursor-pointer" data-bs-toggle="modal"
                                                    data-bs-target="#modalToggle{{$food->id}}"><i class="bx bx-trash me-1"></i>
                                                    {{ __('home.Delete') }}</a>

                                            </div>
                                        </div>
                                        @include('components.modalDelete', [
                                            'action' => 'coach.food.destroy',
                                            'name' => $food->name,
                                            'title' => __('home.Are You Delete'),
                                            'modalToggle' => "modalToggle$food->id",
                                            'id' => $food->id,
                                        ])
                                    </td>
                                </tr>
        ...
        $(document).ready(function() {
            ...
            $('#deleteSelected').click(function() {
                ...
                if (selectedIds.length > 0) {
                    $.ajax({
                        url: "{{ route('coach.food.deleteSelected') }}",
                        type: "POST",
