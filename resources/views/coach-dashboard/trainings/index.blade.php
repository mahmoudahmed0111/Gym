        ...
        <div class="p-3 container-p-y">
            <div class="card">
                <div class="d-flex align-item-center p-4 justify-content-between w-100">
                    <h5 class="card-header p-0">{{ __('home.trainings') }}</h5>
                        ...
                        <tbody>
                            @foreach ($data as $training)
                                <tr>
                                    <th scope="row">
                                        <div class="d-flex align-items-center gap-2">
                                            <input class="form-check-input row__check" type="checkbox" value="{{ $training->id }}" />
                                            <img  src="{{ image_url( $training->image) }}" alt
                                    ...
                                    <td class="">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item" href="{{ route('coach.trainings.edit', $training->id) }}"><i
                                                        class="bx bx-edit-alt me-1"></i> {{ __('home.Edit') }}</a>
                                                <a class="dropdown-item cursor-pointer" data-bs-toggle="modal"
                                                    data-bs-target="#modalToggle{{$training->id}}"><i class="bx bx-trash me-1"></i>
                                                    {{ __('home.Delete') }}</a>

                                            </div>
                                        </div>
                                        @include('components.modalDelete', [
                                            'action' => 'coach.trainings.destroy',
                                            'name' => $training->name,
                                            'title' => __('home.Are You Delete'),
                                            'modalToggle' => "modalToggle$training->id",
                                            'id' => $training->id,
                                        ])
                                    </td>
                                </tr>
        ...
        $(document).ready(function() {
            ...
            $('#deleteSelected').click(function() {
                ...
                if (selectedIds.length > 0) {
                    $.ajax({
                        url: "{{ route('coach.trainings.deleteSelected') }}",
                        type: "POST",
