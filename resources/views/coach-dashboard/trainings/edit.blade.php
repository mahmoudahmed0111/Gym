            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card mb-4 ">
                        ...
                        <div class="card-body">
                            <form action="{{ route('coach.trainings.update',$training->id) }}" method="POST" class="row" enctype="multipart/form-data">
                                @method("put")
                                @csrf
                                ...
                                <div class="col-md-6">
                                    @include('components.input', [
                                        'value' => old('desc',$training->iframe),
                                        'name' => 'iframe',
                                        'type' => 'text',
                                        'label' => 'iframe',
                                    ...
                                    <select id="trainee_ids" name="trainee_ids[]" class="form-control" multiple required>
                                        @foreach($trainees as $trainee)
                                            <option value="{{ $trainee->id }}"
                                                {{ in_array($trainee->id, $training->trainees->pluck('id')->toArray()) ? 'selected' : '' }}>
                                                {{ $trainee->name }}
                                            </option>
                                        @endforeach
                                    </select>
