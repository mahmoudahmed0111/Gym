        ...
        <div class="p-3 container-p-y">
            <div class="card">
                <div class="d-flex align-item-center p-4 justify-content-between w-100">
                    <h5 class="card-header p-0">{{ __('home.vitamin') }}</h5>
                    <div class="d-flex align-item-center gap-3">
                        <button id="deleteSelected" class="btn btn-danger">
                            <i class="fa-regular fa-trash-can"></i>
                        </button>
                        <a href="{{ route('coach.vitamins.create') }}">
                        ...
                        <tbody>
                            @foreach ($data as $vitamin)
                                <tr>
                                    <th scope="row">
                                        <div class="d-flex align-items-center gap-2">
                                            <input class="form-check-input row__check" type="checkbox" value="{{ $vitamin->id }}" />
                                            {{ $vitamin->name }}
                                        </div>
                                    </th>
                                    <td> {{ $vitamin->desc }} </td>
                                    <td> {{ image_url( $vitamin->image) }}</td>
                                    <td> {{ $vitamin->price }} </td>
                                    <td class="">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item" href="{{ route('coach.vitamins.edit', $vitamin->id) }}"><i
                                                        class="bx bx-edit-alt me-1"></i> {{ __('home.Edit') }}</a>
                                                <a class="dropdown-item cursor-pointer" data-bs-toggle="modal"
                                                    data-bs-target="#modalToggle{{$vitamin->id}}"><i class="bx bx-trash me-1"></i>
                                                    {{ __('home.Delete') }}</a>

                                            </div>
                                        </div>
                                        @include('components.modalDelete', [
                                            'action' => 'coach.vitamins.destroy',
                                            'name' => $vitamin->name,
                                            'title' => __('home.Are You Delete'),
                                            'modalToggle' => "modalToggle$vitamin->id",
                                            'id' => $vitamin->id,
                                        ])
                                    </td>
                                </tr>
        ...
        $(document).ready(function() {
            ...
            $('#deleteSelected').click(function() {
                ...
                if (selectedIds.length > 0) {
                    $.ajax({
                        url: "{{ route('coach.vitamins.deleteSelected') }}",
                        type: "POST",
