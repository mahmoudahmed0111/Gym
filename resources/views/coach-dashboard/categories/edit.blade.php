                ...
                <div class="col-md-12">
                    <div class="card mb-4 ">
                        <div class="d-flex p-3 px-4 align-items-center justify-content-between w-100">
                            <h5 class="card-header p-0">{{ __('home.Edit') }} {{ __('home.categories') }}</h5>
                        ...
                        <div class="card-body">
                            <form action="{{ route('coach.categories.update',$data->id) }}" method="POST" class="row" enctype="multipart/form-data">
                                @method("put")
                                @csrf
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select id="type" name="type" class="form-control" required>
                                        <option value="category" {{ old('type', $data->type) == 'category' ? 'selected' : '' }}>Category</option>
                                        <option value="training" {{ old('type', $data->type) == 'training' ? 'selected' : '' }}>Training</option>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select name="trainee_ids[]" id="trainees" class="form-control" multiple>
                                        @foreach($trainees as $trainee)
                                            <option value="{{ $trainee->id }}" {{ in_array($trainee->id, old('trainee_ids', $data->trainees->pluck('id')->toArray())) ? 'selected' : '' }}>
                                                {{ $trainee->name }}
                                            </option>
                                        @endforeach
                                    </select>
