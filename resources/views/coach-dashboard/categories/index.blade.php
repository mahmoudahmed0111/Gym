...
@section('header__title', __('home.main-categories'))
        ...
        <div class="p-3 container-p-y">
            <div class="card">
                <div class="d-flex align-item-center p-4 justify-content-between w-100">
                    <h5 class="card-header p-0">{{ __('home.main-categories') }}</h5>
                    <div class="d-flex align-item-center gap-3">
                        <button id="deleteSelected" class="btn btn-danger">
                            <i class="fa-regular fa-trash-can"></i>
                        </button>
                        <a href="{{ route('coach.categories.create') }}">
                        ...
                        <tbody>
                            @foreach ($data as $category)
                                <tr>
                                    <th scope="row">
                                        <div class="d-flex align-items-center gap-2">
                                            <input class="form-check-input row__check" type="checkbox" value="{{ $category->id }}" />
                                    ...
                                    <td class="">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item" href="{{ route('coach.categories.edit', $category->id) }}"><i
                                                        class="bx bx-edit-alt me-1"></i> {{ __('home.Edit') }}</a>
                                                <a class="dropdown-item cursor-pointer" data-bs-toggle="modal"
                                                    data-bs-target="#modalToggle{{$category->id}}"><i class="bx bx-trash me-1"></i>
                                                    {{ __('home.Delete') }}</a>
                                                    <a class="dropdown-item" href="{{ route('coach.categories.show', $category->id) }}"><i
                                                        class="bx bx-show me-1"></i> {{ __('home.Show') }}</a>
                                            </div>
                                        </div>
                                        @include('components.modalDelete', [
                                            'action' => 'coach.categories.destroy',
                                            'name' => $category->name,
                                            'title' => __('home.Are You Delete'),
                                            'modalToggle' => "modalToggle$category->id",
                                            'id' => $category->id,
                                        ])
                                    </td>
                                </tr>
        ...
        $(document).ready(function() {
            ...
            $('#deleteSelected').click(function() {
                ...
                if (selectedIds.length > 0) {
                    $.ajax({
                        url: "{{ route('coach.categories.deleteSelected') }}",
                        type: "POST",
