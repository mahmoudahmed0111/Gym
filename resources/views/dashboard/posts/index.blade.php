...
@section('header__title', __('home.posts'))
@section('header__icon', 'fa-solid fa-share')
@section('main')
        ...
        <div class="p-3 container-p-y">
            <div class="card">
                <div class="d-flex align-item-center p-4 justify-content-between w-100">
                    <h5 class="card-header p-0">{{ __('home.posts') }}</h5>
                        ...
                        <tbody>
                            @foreach ($data as $post)
                                <tr>
                                    <th scope="row">
                                        <div class="d-flex align-items-center gap-2">
                                            <input class="form-check-input row__check" type="checkbox" value="{{ $post->id }}" />
                                        {{$post->id  }}</td>
                                        </div>
                                    </th>
                                    <td>
                                        <img src="{{ image_url( $post->user->img) }}" alt
                                    ...
                                    <td>
                                        @if($post->img)
                                        <img src="{{ image_url( json_decode($post->img)[0]->path) }}" alt
                                        class="w-px-50 h-auto rounded-circle" />
                                        @elseif($post->video)
                                        <a href="{{asset("storage/".$post->video)}}" class="btn btn-light">video</a>
                                        @else
                                        N/A
                                        @endif
                                    </td>
                                    ...
                                    <td class="">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item cursor-pointer" data-bs-toggle="modal"
                                                    data-bs-target="#modalToggle{{$post->id}}"><i class="bx bx-trash me-1"></i>
                                                    {{ __('home.Delete') }}</a>
                                            </div>
                                        </div>
                                        @include('components.modalDelete', [
                                            ...
                                            'modalToggle' => "modalToggle$post->id",
                                            'id' => $post->id,
                                        ])
                                    </td>
                                </tr>
        ...
        $(document).ready(function() {
            ...
            $.fn.dataTable.ext.search.push(
                function(settings, data, dataIndex) {
                    ...
                    var dateColumn = data[4]; // Assuming the date column is the 2nd column (index 1)
            ...
            $('#deleteSelected').click(function() {
                ...
                if (selectedIds.length > 0) {
                    $.ajax({
                        url: "{{ route('posts.deleteSelected') }}",
                        type: "POST",
