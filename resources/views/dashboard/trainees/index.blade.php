                        ...
                        <tbody>
                            @foreach ($data as $trainee)
                                <tr>
                                    ...
                                    <td>{{ $trainee->coach ? $trainee->coach->name : 'No coach Assigned' }}</td>
                                    ...
                                    <td class="">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item" href="{{ route('trainees.edit', $trainee->id) }}"><i
                                                ...
                                                <a class="dropdown-item cursor-pointer" data-bs-toggle="modal"
                                                    ...
                                                    {{ __('home.Delete') }}</a>
                                                <a class="dropdown-item" href="{{ route('trainees.show', $trainee->id) }}"><i
                                                        class="bx bx-show me-1"></i> {{ __('home.Show') }}</a>
                                            </div>
                                        </div>
                                        @include('components.modalDelete', [
                                            'action' => 'trainees.destroy',
                                            'name' => $trainee->name,
        ...
        $(document).ready(function() {
            ...
            $('#deleteSelected').click(function() {
                ...
                if (selectedIds.length > 0) {
                    $.ajax({
                        url: "{{ route('trainees.deleteSelected') }}",
                        type: "POST",
            ...
            $('.toggle-activation').change(function() {
                ...
                $.ajax({
                    url: "{{ route('trainees.toggleActivation') }}",
                    type: "POST",
                    data: { id: traineeId },
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
